...
Attribution is bounded and self-limiting: 60 sessions per Tier-2 run, 20 per background refresh (`daily_catchup` = `incremental` + the four bounded per-item passes — gear, splits, HRV and daily summary — the inline paths stay on plain `incremental` so nothing a user waits for gets slower), and the work list is "activities with no gear link", so it empties and then costs nothing. Sessions that genuinely carry none (manual entries, most strength work) are remembered in `sync_state['gear_no_link']` after a 7-day settling period — before that they are retried, because Garmin may not have attached the default pair when we first asked. `capability_map['gear']` records an empty locker as a normal account state, so the tool can say so instead of rendering an empty table.


**The same tiering applies to every per-item pass.** Per-lap splits (§3.2 #12), the HRV gap-fill and the daily-summary gap-fill are all one call per item with a work list that empties, so they ride Tier 2, the CLI sync and the background refresh — 10/20/20 per background run against 40/60/60 in the foreground. `incremental` ingests a new activity without its laps, so without the background pass `activity_laps` grows only where someone runs a backfill by hand.
---
