...
uv run fartlek sync --since 2025-11-14   # one floor date for the whole pass (D12); re-run until nothing is 'left'
uv run fartlek-mcp               # MCP server over stdio
```

**A real test account is installed.** `~/.fartlek/` contains the Garmin tokens and the store for account `b2db9a6f-...`: **279 days (2025-11-13 → today), 434 activities, 3,174 laps, 163 nights of sleep timeline**, persisted HR zones and weight. This is the raw material for all live verifications. The history was deepened on 2026-08-18 with `fartlek sync --since 2025-11-14` (see §7, D12): 2025-11-14 is the athlete's first night on Garmin, so the store now starts at the account's own floor.
...
- **D10** (2026-08-18): `backfill_splits()` had no production caller at all — laps existed only where someone had fetched them by hand. It now runs on every `fartlek sync`. In the same pass, `garmin_sync(backfill_days=N)` gained the full history pass (tier1 over the same window, then the wellness gap-fills and splits): it previously ran `incremental() + tier2()`, so **MCP depth requests deepened sleep and nothing else**. **Trap**: merge tier1's stats under distinct keys — its `activities` count is history size, not `new_activities`.
- **D11** (2026-08-18): gear attribution trusted its first answer forever. Garmin attaches the **default pair at upload**, so the athlete's correction in Connect never reached the store and the rotation credited kilometres to the wrong shoe. `_GEAR_RECHECK_DAYS` (7d) now re-asks recent links; outside that window a link stays final. The attribution window also follows `activity_history_days()` instead of a hard 180. **Trap**: absence-only work lists are right for laps and wellness (they never change retroactively) and wrong for gear, which the athlete edits — don't copy the pattern blindly.
- **D12** (2026-08-18): every backfill in the engine is parameterised by a *depth in days*, and each entry point kept its own default — so a store could only ever be as deep as the shallowest one. `tier2_cursor` marks itself `done` at whatever floor the first cold start picked and **never reaches further back on its own**; `backfill_splits()` stayed at a hard `SPLITS_HISTORY_DAYS` (120) even on a widened history, leaving months of activities with no laps. `fartlek sync --since YYYY-MM-DD` now sets one floor date for the whole pass (tier1, splits, gear, tier2, HRV, daily summary), clamped so a shallow `--since` can never shrink a window below its default. **Trap**: the per-step conventions differ by a day (tier1/tier2 subtract the count from today, `backfill_splits` counts today in), which is why `_span_days` returns `(today - since).days + 1` — the strictest reading, a harmless superset for the rest.
- **Garmin retention — the sleep phase timeline is NOT backfillable.** `/wellness-service/wellness/dailySleepData/{name}` keeps answering for arbitrarily old dates, but `sleepLevels` is dropped after **~140 days**: probed on 2026-08-18, present for 2026-04-01 and later, `None` for 2026-03-01 and every date before it, while `dailySleepDTO` (score, stages in hours, HRV, RHR) comes back in full for all of them. So `days` sleep columns heal retroactively and `sleep_timeline` (the input to SRI/social-jet-lag) only ever accrues forward — a store that stops syncing loses those nights for good. Same ~140-day rule the `backfill_hrv` docstring already notes for the overnight HRV series.
