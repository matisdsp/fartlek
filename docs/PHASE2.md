...
**Phase 2 is closed** (2026-07-27): engine, tools, prompts/resources and CI gates all ✅, v0.2.2 live on PyPI and the MCP registry. What is still ⬜ below is the deferred heavy eval programme (§4), two open questions (§5) and the third-party directory submissions (§7) — none of them Phase-2 blockers. Next build is Phase 3, the write path.
...
| Retroactive precedent mining | §3.2 #5 | ✅ `analytics/precedent.py` — cross-source merge; external episodes excludable. Trigger levels calibrated on the athlete's own categorized episodes (values kept in the maintainer's private notes) |
...
**Scope revised 2026-07-23 → v0.2 ships with automated CI gates + a reduced eval harness; the heavy manual programme (30 tasks × 3 clients, transcript audits) moves to a later v0.2.x.** All CI gates below are done; the eval-harness rows marked *(deferred)* did not land in 0.2.1 or 0.2.2 and remain the main open quality debt.
...
| Eval harness ~30 multi-tool tasks, Claude Code + Desktop + Cursor | ⬜ *(deferred)* |
| Token + calls-per-task regression gates | ⬜ *(deferred)* |
| Transcript audits (every LLM-re-derived number = missing pre-computation) | ⬜ *(deferred)* |
| French-language eval tasks (server renders English, client translates) | ✅ v0.2 reduced: task D (`garmin_load`) answered in French, all numbers preserved (`docs/EVAL.md`). Full FR set → *(deferred)* |
...
| Anomaly-scanner threshold tuning on real multi-month data | ✅ 75 → 27 alerts, AMBER 27 → 4, anchored by a certified real illness positive |
...
| D6 | Rows written by a mid-day sync stayed frozen at their mid-day values forever (2026-07-20 held 6,847 steps vs an actual 18,664) | ✅ fixed by the same range backfill, which rewrites completed days — **but only for the 9 metrics userstats serves**; the columns with no range endpoint stayed frozen until D9 |
...
| D9 | Three wellness families only ever accrued **forward**, never backward: `hrv_last_night/_weekly_avg/_status` came from `/hrv-service/hrv/{today}` alone, so any day without a sync stayed blank for good; `max_hr`, `max_stress`, `spo2_avg` and `body_battery_wake` have no range endpoint and came from the daily summary fetched for today only (D6's residue — on the real account `max_hr` held 12 rows, one per day a sync happened, each a frozen mid-day value); and body battery walked a hard-coded 90d in 3 chunks, so widening `FARTLEK_ACTIVITY_HISTORY_DAYS` left the column blank over the extra months | ✅ fixed 2026-08-18 — `store.dates_missing_metric()` + `backfill_hrv()` / `backfill_daily_summary()` on the `backfill_splits` pattern (capped per run, resumable by construction, no cursor); tier 1 re-asks **yesterday** once per sync to thaw the frozen row, placed before the range calls so those still win on the columns they own; body-battery chunks derive from `activity_history_days()`. Verified on the real account over 279 days: `hrv_last_night` 161 → 274, `body_battery_high` 119 → 279, `max_hr`/`max_stress` 12 → 279, 0 remaining gaps beyond genuine watch-not-worn days. +8 hermetic tests |
| D10 | Two paths never reached production: `backfill_splits()` had **no caller outside tests**, so per-lap data (§3.2 #12 — the input to every pace-band and EF analysis) only existed for activities fetched by hand; and `garmin_sync(backfill_days=N)` ran `incremental() + tier2()` only, so a depth request through MCP deepened sleep and left activities, userstats scalars, weight and body battery at whatever depth the last CLI sync reached | ✅ fixed 2026-08-18 — splits run on every `fartlek sync` (capped, self-healing: the work list is 'activities with no stored laps'); `run_sync` now runs `tier1(history_days=N)` → tier2 → wellness gap-fills → splits, merging stats under distinct keys so tier1's activity count cannot clobber `incremental`'s `new_activities`. `tier1()` takes an optional window clamped to never go below `activity_history_days()`. Verified on the real account: 35 running sessions had no laps → 458 laps recovered, second run 0 calls. +4 hermetic tests |
| D11 | Gear attribution never revisited a session that already had a link (the work list was `activities with no gear link`), but Garmin attaches the **default pair at upload** — so the first answer is exactly the one the athlete corrects in Connect minutes later, and the correction never reached the store. Separately, `GEAR_HISTORY_DAYS` was a hard 180 while its own comment claimed it 'matches the activity history': once D2 made the history configurable, a widened window ingested sessions the gear pass could never reach | ✅ fixed 2026-08-18 — `_GEAR_RECHECK_DAYS` mirrors `_GEAR_SETTLE_DAYS` (same 7-day window in which a link is not yet trustworthy, applied to the opposite case; outside it a link stays final, so retiring a pair cannot rewrite last month); attribution window derives from `activity_history_days()`. Caught on the real account: two sessions of 14/08 stored as Hoka Bondi 9 II where Garmin reported ASICS Superblast 3 and Brooks Glycerin, and 137 stranded activities (83 runs, 13/11/2025→18/02/2026). Post-fix the per-pair counters match Garmin exactly on 5 of 6 pairs, the sixth off by one, that  being one session predating the store. +4 tests |
| D12 | Every backfill in the engine takes a *depth in days* and each entry point kept its own default, so a store could only ever be as deep as its shallowest step. `tier2_cursor` stamps itself `done` at whatever floor the first cold start chose and never reaches further back on its own; `backfill_splits()` stayed pinned to `SPLITS_HISTORY_DAYS` (120) even when D2 had widened the activity history, so months of ingested sessions carried no laps at all — the input to every pace-band and EF analysis (§3.2 #12) | ✅ fixed 2026-08-18 — `fartlek sync --since YYYY-MM-DD` sets one floor date for the whole pass (tier1, splits, gear, tier2, HRV, daily summary). `cli._span_days()` converts it to `(today - since).days + 1`, the strictest of the engine's per-step day conventions, and every call site clamps so a shallow `--since` can never shrink a window below its default. Verified on the real account (floor 2025-11-14, the athlete's first night on Garmin): activities/days/PMC now start 2025-11-13 (279 dense days, 434 activities), laps 2,646 → 3,174 with the Nov–Dec 2025 block recovered (0 running sessions left without laps), gear attribution reaching the same floor. **Not recoverable**: `sleepLevels` is dropped by Garmin after ~140 days, so `sleep_timeline` still starts 2026-03-01 while the sleep DTO columns backfill in full. +5 hermetic tests |
...
✅ v0.2.0, v0.2.1 and v0.2.2 released — tagged, pushed, on PyPI via the OIDC workflow
✅ Official MCP registry — 0.2.2 published via `mcp-publisher`, `isLatest: true` (2026-07-24, re-verified 2026-07-27)
