...
async def test_fixed_time_goal_takes_a_distance_target(store):
    ...
    ctx = FakeContext(store)
    out = await set_profile.run(
        ctx, goal_distance="24h", goal_race_date="2026-09-05", goal_target_km=180.0,
    )
    assert "24h" in out and "180 km" in out
    profile = store.get_profile()
    assert profile["goal_distance"] == "24h"
    assert float(profile["goal_target_km"]) == 180.0


async def test_target_km_is_rejected_for_a_distance_race(store):
    ctx = FakeContext(store)
    out = await set_profile.run(ctx, goal_distance="marathon", goal_target_km=180.0)
